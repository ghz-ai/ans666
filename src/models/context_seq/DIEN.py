...
class DIENBase(DINBase):
    ...
    def _define_init_dien(self, args, corpus):
        ...
        self.gru_emb_size = self.embedding_size * (
                    len(self.item_context) + self.add_historical_situations * len(self.situation_context))
        self.fcn_embedding_size = self.embedding_size * (
                    len(self.user_context) + len(self.situation_context) + len(self.item_context)) + \
                                  self.gru_emb_size * 3
    ...
    def _define_params_DIEN(self):
        ...
        for f in self.user_context + self.item_context + self.situation_context:
            self.embedding_dict[f] = nn.Embedding(self.feature_max[f], self.embedding_size) if f.endswith(
                '_c') or f.endswith('_id') else \
    ...
    def get_all_embeddings(self, feed_dict):
        history_emb, target_emb, user_feats_emb, situ_feats_emb = DINBase.get_all_embedding(self, feed_dict,
                                                                                            merge_all=False)
    ...
    def get_neg_embeddings(self, feed_dict):
        history_item_emb = torch.stack(
            [self.embedding_dict[f](feed_dict['history_neg_' + f]) if f.endswith('_c') or f.endswith('_id')
        ...
        if self.add_historical_situations:
            history_situ_emb = torch.stack(
                [self.embedding_dict[f](feed_dict['history_' + f]) if f.endswith('_c') or f.endswith('_id')
    ...
    def target_attention(self, target_emb, interest_emb, lengths):
        ...
        attention_prod = torch.bmm(interest_emb,
                                   self.attentionW.unsqueeze(dim=0).repeat(batch_size, 1, 1))  # batch * len * emb size
    ...
    def interest_evolving(self, interests, interest_emb, target_emb, lengths):
        ...
        if self.evolving_gru_type == 'AIGRU':
            packed_inputs = nn.utils.rnn.pack_padded_sequence(interest_emb * attention.unsqueeze(dim=-1),
                                                              lengths=lengths.cpu(), batch_first=True,
    ...
    def _get_inp(self, feed_dict):
        ...
        neg_history_emb = self.get_neg_embeddings(feed_dict) if self.alpha_aux > 0 and feed_dict[
            'phase'] == 'train' else None

        # repeat and flatten history according to multiple targets
        history_emb_2d = history_emb.unsqueeze(dim=1).repeat(1, item_num, 1, 1).reshape(-1, history_emb.shape[-2],
                                                                                        history_emb.shape[-1])
        ...
        h_out = self.interest_evolving(gru_output, interest_emb, target_emb_2d, hislens_2d).reshape(batch_size,
                                                                                                    item_num, -1)
        ...
        return inp, {'neg_history': neg_history_emb, 'pos_history': history_emb,
                     'interest_emb': interest_emb.reshape(batch_size, item_num, history_emb.size(1), -1)[:, 0, :,
                                     :].squeeze(dim=1),
                     'lengths': hislens}
        ...
        def _get_feed_dict_dien(self, index, feed_dict):
            if self.model.alpha_aux > 0 and self.phase == 'train':
                ...
                for c in self.corpus.item_feature_names:
                    feed_dict['history_neg_' + c] = np.array(
                        [self.corpus.item_features[iid][c] if iid in self.corpus.item_features else 0
